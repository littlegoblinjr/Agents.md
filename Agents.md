...
```mermaid
graph TD
    Input([System Input]) --> Guardrail["🛡️ Guardrail Agent"]
    Guardrail --> Planner["📅 Planner Agent"]
    Planner --> DAG{Task DAG}
    DAG --> Market["📊 Market Agent"]
    DAG --> Tech["💻 Tech Agent"]
    DAG --> Finance["💰 Finance Agent"]
    DAG --> Pitch["🎤 Pitch Agent"]
    Market & Tech & Finance & Pitch --> Synthesis["🧩 Synthesis Agent"]
    Synthesis --> Evaluation["⚖️ Evaluation Agent"]
    Evaluation --> DB[("Neon DB (PostgreSQL)")]
    Evaluation --> UI["Real-Time Telemetry Dashboard"]
```
...
```mermaid
graph LR
    User((User Voice)) --> STT["STT Pipeline"]
    STT --> LLM["OpenAI Reasoning"]
    LLM --> Persona["Personality Heuristics"]
    Persona --> TTS["Edge TTS"]
    TTS --> Voiceout((Audio Output))
```
